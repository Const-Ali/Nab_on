...
import { validNationalId, validBirthDate } from '../../app-lib'
...
export function CreateModal({ existing, creatorName, onClose, onCreate }: { existing: Person[]; creatorName: string; onClose: () => void; onCreate: (p: Person) => void }) {
  ...
  const submit = (e: FormEvent) => {
    ...
    if (existing.some(p => normalizeDigits(p.nationalId) === id)) { setError('این کد ملی قبلاً ثبت شده است.'); return }
    const birth = birthDate.trim() ? normalizeDigits(birthDate.trim()) : ''
    if (birth && !validBirthDate(birth)) { setError(`تاریخ تولد معتبر نیست. مثال: ${jalaliToday().slice(0, 5)}02/12`); return }
    ...
    onCreate({ id: `P-${maxId + 1}`, firstName: firstName.trim(), lastName: lastName.trim(), nationalId: id, birthDate: birth || '—', fatherName: fatherName.trim(), photo: photo || undefined, deadline: dl || undefined, status: 'درخواست پرونده', updatedAt: nowStamp(), updatedBy: creatorName })
  }
  return (
    <div className="backdrop" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <form className="modal" onSubmit={submit}>
        ...
        <div className="form-grid">
          ...
          <label>مهلت پیگیری (اختیاری)<BirthDateField value={deadline} onChange={setDeadline} /></label>
        </div>
