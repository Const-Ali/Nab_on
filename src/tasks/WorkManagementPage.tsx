...
import { load, nowStamp, nowTs, save, uid } from '../shared'
...
export default function WorkManagementPage({ users, sessionUsername, sessionName }: Props) {
  // هر کاربر یک ذخیره‌ی کاملاً مستقل دارد؛ داشبوردها هیچ اتصالی به هم ندارند.
  ...
  const [tasks, setTasks] = useState<WorkTask[]>(() => {
    ...
    // یک‌بار مهاجرت از ذخیره‌ی مشترک نسخه‌ی قبلی: فقط کارهای مربوط به همین کاربر
  ...
  useEffect(() => { save(storeKey, tasks) }, [storeKey, tasks])
  ...
  // این استور فقط مال همین کاربر است؛ آنچه ذخیره شده همان داشبورد اوست.
  const mine = tasks
  ...
  const filtered = useMemo(() => {
    const quickOk = (t: WorkTask) =>
      quick === 'all' ? true
        : quick === 'today' ? isDueToday(t)
          : quick === 'in_progress' ? t.status === 'in_progress'
            : quick === 'completed' ? t.status === 'completed'
  ...
  const isFiltering = quick !== 'all' || filters.status !== 'all' || filters.priority !== 'all'
    || filters.category !== 'all' || Boolean(filters.q)
  ...
  const saveTask = (fields: TaskFields) => {
    if (editing) {
      ...
      if (fields.priority !== editing.priority) logs.push(`اولویت از «${taskPriorityLabels[editing.priority]}» به «${taskPriorityLabels[fields.priority]}» تغییر کرد`)
      ...
      const completedAt = fields.status === 'completed' && editing.status !== 'completed' ? nowStamp() : fields.status !== 'completed' ? undefined : editing.completedAt
      setTasks(cur => cur.map(t => (t.id === editing.id ? { ...t, ...fields, completedAt, history: [...t.history, ...logs.map(log)] } : t)))
    } else {
      const task: WorkTask = {
        id: uid('TASK'),
        ...fields,
  ...
  const toggleDone = (t: WorkTask) => {
    ...
    setTasks(cur => cur.map(x => (x.id === t.id
      ? { ...x, status: done ? 'todo' : 'completed', completedAt: done ? undefined : nowStamp(), history: [...x.history, log(done ? 'کار بازگشایی شد' : 'کار تکمیل شد')] }
      : x)))
  }
  ...
  return (
    <>
      <Heading
        ...
        subtitle="کارهای شخصی و سازمانی شما — این بخش خصوصی است و فقط خودتان به آن دسترسی دارید."
        action={<button className="primary" onClick={() => { setEditing(null); setFormOpen(true) }}><Icon name="plus" /> ایجاد کار جدید</button>}
      />
      ...
      {detailTask && (
        <TaskDetails
          ...
          onToggle={toggleDone}
          onEdit={t => { setDetailId(null); setEditing(t); setFormOpen(true) }}
          onDelete={deleteTask}
        />
      )}
    </>
