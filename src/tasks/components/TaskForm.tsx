...
export default function TaskForm({ initial, typeDefault, categories, onClose, onSave }: Props) {
  ...
  const submit = (e: FormEvent) => {
    e.preventDefault()
    if (!title.trim()) { setError('عنوان کار الزامی است.'); return }
    const start = startDate.trim() ? normalizeDigits(startDate.trim()) : ''
    const due = dueDate.trim() ? normalizeDigits(dueDate.trim()) : ''
    if (start && !validJalaliDate(start)) { setError('تاریخ شروع معتبر نیست.'); return }
    if (due && !validJalaliDate(due)) { setError('مهلت انجام معتبر نیست.'); return }
  ...
  return (
    <div className="backdrop" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <form className="modal wm-modal" onSubmit={submit}>
        ...
        <label>عنوان کار *<input autoFocus value={title} onChange={e => { setTitle(e.target.value); setError('') }} placeholder="مثلاً تهیه گزارش هفتگی" /></label>
        <label>توضیحات<textarea value={description} onChange={e => setDescription(e.target.value)} rows={2} placeholder="شرح کوتاه کار..." /></label>
        <div className="form-grid">
          ...
          <label>تاریخ شروع<BirthDateField value={startDate} onChange={setStartDate} /></label>
          <label>مهلت انجام<BirthDateField value={dueDate} onChange={setDueDate} /></label>
          <label>دسته‌بندی<input value={category} onChange={e => setCategory(e.target.value)} list="wm-cats" placeholder="مثلاً اداری، گزارش..." /></label>
          <label>برچسب‌ها<input value={tagsText} onChange={e => setTagsText(e.target.value)} placeholder="با «،» جدا کنید: فوری، پیگیری" /></label>
        </div>
        <datalist id="wm-cats">{categories.map(c => <option key={c} value={c} />)}</datalist>
        <label>توضیحات تکمیلی<textarea value={note} onChange={e => setNote(e.target.value)} rows={2} placeholder="یادداشت داخلی، لینک، جزئیات..." /></label>
