      ...
      <div className="wm-filters-top">
        <div className="search">
          ...
          <input
            ...
            placeholder="جستجو در عنوان، توضیحات، دسته یا برچسب..."
          />
        </div>
        <div className="wm-view-toggle">
          <button
            type="button"
            className={value.view === 'list' ? 'on' : ''}
          ...
          <button
            type="button"
            className={value.view === 'kanban' ? 'on' : ''}
          ...
          <button
            type="button"
            className={value.view === 'gantt' ? 'on' : ''}
            onClick={() => onChange({ view: 'gantt' })}
            title="نمای گانت‌چارت و تایم‌لاین"
          >
            📊 گانت‌چارت
          </button>
        </div>
        <button type="button" className="wm-filters-toggle" onClick={() => setOpen(o => !o)}>
          {open ? 'بستن فیلترها ▲' : 'فیلترها ▼'}
        </button>
      </div>
      <div className={`wm-filters-body ${open ? 'open' : ''}`}>
        <select value={value.status} onChange={e => onChange({ status: e.target.value })}>
          {taskStatusFilterOptions.map(([v, l]) => (
        ...
        <select value={value.priority} onChange={e => onChange({ priority: e.target.value })}>
          {taskPriorityFilterOptions.map(([v, l]) => (
            <option key={v} value={v}>
              {l}
            </option>
          ))}
        ...
        {(value.status !== 'all' || value.priority !== 'all' || value.category !== 'all' || value.q) && (
          <button
            type="button"
            className="mini-btn"
            onClick={() => onChange({ q: '', status: 'all', priority: 'all', category: 'all' })}
          >
            پاک کردن فیلترها ✕
          </button>
        )}
